...
export default function Email() {
    ...
    <Flex 
      ...
      w="60%"
        ...
        <Stack spacing={4} direction={{ base: 'column' }} w={'full'}>
          <Text fontSize={20} fontWeight={500}>Subject</Text>
          <Input 
            type={'text'}
            placeholder={'Subject...'}
            color={useColorModeValue('gray.800', 'gray.200')}
            ...
            />
            <Text  fontSize={20} fontWeight={500}>Mail Body</Text>
          <Textarea
            type={'text'}
  ...
  <Table variant='striped'>
    ...
    <Tbody>
      {allContact.length && allContact.map((data)=>(
        <Tr>
          <Td><Checkbox value={data.c_email} onChange={({target})=>handleChange(target)} /></Td>
          <Td>{data.c_name}</Td>
          <Td>{data.c_email}</Td>
        </Tr>
      ))}
...
<Button
            ...
            flex={'1 0 auto'}
            _hover={{ bg: 'blue.500' }}
            _focus={{ bg: 'blue.500' }} onClick={handlesendEmail}>
            Send Email
          </Button>
    </Flex>
  );
